...
namespace SquareWidget.Astronomy.Core.Calculators
{
    ...
    {
        public static List<HourAngle> Calculate(Degrees φ)
        {
            List<HourAngle> list = new();
            
            double L = Math.Sin(φ * 2 * Math.PI / 360);

            list.Add(new HourAngle(12, new Degrees(0))); // solar noon
            ...
            for (int t = 13; t <= 18; t++)
            {
                ...
                HourAngle afternoon = new(t, new Degrees(H));
                HourAngle morning = new(morningHour, new Degrees(-H));
